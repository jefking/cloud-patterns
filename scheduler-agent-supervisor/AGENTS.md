...
## Scope
These instructions apply to every file in this folder. Build only software, examples, tests, and documentation that implement the Scheduler Agent Supervisor pattern. Treat this file as the architectural contract for this subtree.

## Source Material
- Microsoft Learn pattern page: https://learn.microsoft.com/en-us/azure/architecture/patterns/scheduler-agent-supervisor
...
## Pattern Intent
Coordinate distributed actions through a scheduler, worker agents, and a supervisor that monitors and recovers progress.
...
- A workflow has multiple distributed actions that need scheduling, ownership, monitoring, and recovery.
- Agents can execute steps independently while a supervisor tracks health and completion.
- Workflow state can be persisted so work can resume after failure.

Do not use this pattern when:
- The task is just cron scheduling without distributed step supervision.
- All work happens synchronously in one process and has no recovery boundary.
- No durable record exists for assigned, running, completed, failed, or abandoned work.

## Architecture Invariants
- Scheduler, agent, and supervisor responsibilities are separated.
- Workflow step state, ownership, retries, and deadlines are durable.
- Agents execute idempotent or duplicate-safe actions.
- The supervisor can detect timeout, crash, stuck work, and failed agents.
...
- Define retry, reassignment, compensation, abandonment, and escalation rules.
- Use heartbeats, leases, or checkpoints for long-running agent work.
- Expose workflow progress, agent health, stuck steps, and supervisor decisions.
- Test scheduler restart, agent crash, supervisor recovery, and duplicate assignment.

## Pattern Boundaries and Common Confusions
- Leader Election chooses one coordinator; Scheduler Agent Supervisor manages distributed work lifecycle.
- Saga manages distributed transaction consistency; this pattern coordinates general distributed actions.
- A queue plus workers is not enough unless supervision and scheduling semantics are explicit.

## Verification Checklist
- Can work resume after any component restarts?
- Can the supervisor account for every scheduled action?
- Are stuck and partially completed workflows recoverable?
